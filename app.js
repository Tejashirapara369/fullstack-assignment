...
// api endpoints
app.post('/api/g2_test', async (req, res) => { // api to create new API
    try {
...
app.post("/api/updateLicense", async (req, res) => {
    ...
    try {
        const data = await LicenseModel.findOneAndUpdate({ licenseNo: req.body.licenseNo }, { carDetails }, { new: true });
        res.render("updateG2", { data });
    } catch (error) {
        res.send(error);
    }
